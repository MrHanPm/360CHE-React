...
class Clues extends React.Component {
    ...
    FollSidebar(val){
        //console.log(val);
        if(val.s_levelsetstatus.length == 0 && val.s_followstatus.length == 0 && val.s_brandids.length ==0 && val.s_clueslevel.length ==0 && val.s_cheliangyongtuid.length ==0 && val.s_clueresource.length ==0 && val.s_followId == '' && val.s_follownummax == -1 && val.s_follownummin == -1 && val.s_lastlinktimemin == '' && val.s_lastlinktimemax == ''){
            this.setState({pf:0});
        } else {
            this.setState({pf:1});
        }
